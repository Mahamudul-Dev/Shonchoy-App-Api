...
module.exports.sonchoySubmit = async (req, res) => {
  try {
    const data = req.body;
    // default value
    ...
    if (result != null) {
      ...
      const collectionStatusSql = `SELECT sonchoy_collection_status FROM 7_sodosso_vorti WHERE serial = ?`;
      ...
      await db.execute(gatewayCheckSonchoy, [1, serial]);

      if (collectionStatusResult[0][0]["sonchoy_collection_status"] === 30) {
        const sonchoyLastCollectionMonthUpdateSql = `UPDATE 7_sodosso_vorti SET sv_last_collection_month = ? WHERE serial = ?`;

        await db.execute(sonchoyLastCollectionMonthUpdateSql, [
          currentDate,
          serial,
        ]);
      }
...
module.exports.bokeyaSonchoySubmit = async (req, res) => {
  try {
    const data = req.body;
    // default value
    const cr_amount = data.sonchoy;
